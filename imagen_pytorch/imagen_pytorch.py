...
class Block(nn.Module):
    def __init__(
        ...
        if zero_proj_out:
            self.project = zero_module(self.project)
...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.block2 = Block(dim_out, dim_out, groups = groups, zero_proj_out=zero_proj_out)
...
class Unet(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.attn_pool = PerceiverResampler(dim = cond_dim, depth = 2, dim_head = attn_dim_head, heads = attn_heads, num_latents = attn_pool_num_latents, max_seq_len = max_text_len, ff_mult=ff_mult) if attn_pool_text else None
        ...
        resnet_klass = partial(ResnetBlock, zero_proj_out=zero_resblock_proj_out)
        ...
        for ind, ((dim_in, dim_out), layer_num_resnet_blocks, groups, layer_attn, layer_attn_depth, layer_cross_attn) in enumerate(zip(in_out, *layer_params)):
            ...
            self.downs.append(nn.ModuleList([
                ...
                resnet_klass(
                    ...
                    groups=groups),
                nn.ModuleList([resnet_klass(current_dim,
                                           current_dim,
        ...
        for ind, ((dim_in, dim_out), layer_num_resnet_blocks, groups, layer_attn, layer_attn_depth, layer_cross_attn) in enumerate(zip(reversed(in_out), *reversed_layer_params)):
            ...
            self.ups.append(nn.ModuleList([
                resnet_klass(
                    ...
                    time_cond_dim=time_cond_dim,
                    groups=groups),
                nn.ModuleList([resnet_klass(dim_out + skip_connect_dim,
                                           dim_out,
                                           cond_dim=inner_cond_dim,
                                           heads=cross_attn_heads,
                                           dim_head=cross_attn_dim_head,
        ...
        self.final_res_block = resnet_klass(final_conv_dim, dim, time_cond_dim = time_cond_dim, groups = resnet_groups[0], use_gca = use_global_context_attn) if final_resnet_block else None
